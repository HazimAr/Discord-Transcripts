    ...
    messagesArray.map(async (message) => {
      ...
      if (message.reference?.messageId) {
        ...
        reference.innerHTML = `<img class="chatlog__reference-avatar" src="${
          author.avatarURL({ dynamic: true }) ?? staticTypes.defaultPFP
      ...
      timestamp.title = he.escape(
        message.createdAt.toLocaleTimeString("en-us", staticTypes.timestampLong)
      );
      ...
      if (message.components.length) {
        for (const row of message.components) {
          ...
          componentDiv.classList.add("chatlog__component");
          componentDiv.style.display = "flex";
          componentDiv.style.flexWrap = "wrap";
          componentDiv.style.gap = "0.5em";
          ...
          for (const component of row.components) {
            if (component.type == "BUTTON") {
              ...
              componentButton.classList.add(
                ...
                `chatlog__component-button--${component.style?.toLowerCase()}`
              );
              if (component.disabled) componentButton.disabled = true;


              if (component.emoji) {
                const componentButtonEmoji = document.createElement("img");
                componentButtonEmoji.classList.add("emoji", "emoji--small");

                componentButtonEmoji.alt =
                  component.emoji.name ?? component.emoji.id ?? "Unknown emoji";

                componentButtonEmoji.src = `https://cdn.discordapp.com/emojis/${
                  component.emoji.id
                }.${component.emoji.animated ? "gif" : "png"}`;

                console.log(componentButtonEmoji.src);
                componentButton.appendChild(componentButtonEmoji);
              }
              
              componentButton.textContent = component.label;
              if (component.url) {
                // TODO: add <a target="_black" href={component.url}></a>
                const componentButtonLink = document.createElement("a");
                componentButtonLink.classList.add(
                  "chatlog__component-button-link"
                );
                componentButtonLink.href = component.url;
                componentButtonLink.target = "_blank";

                componentButton.appendChild(componentButtonLink);
              }
              componentDiv.appendChild(componentButton);
            } else if (component.type == "SELECT_MENU") {
              // i know there is only 2 types but i added it incase discord wants to add more types of components
            }
          }
...
function formatContent(
  ...
  content = purify(content)
    ...
    .replace(/\&lt\;\@\&amp\;([0-9]{16,20})\&gt\;/g, (channel: string) => {
      ...
      }; background-color: ${rgba};" title="${roleInGuild?.name ?? roleId}">@${
        roleInGuild?.name ?? "Unknown Role"
      }</span>`;
    });
